            ...
            <div class="row">
                ...
                <div class="row align-items-center">
                    ...
                    <div class="page-header">
                        <div class="row">
                            <div class="col-sm-12">
                                <h3 class="page-title">កែប្រែព័ត៌មានរបស់ស្នាក់នៅក្នុងវត្ត</h3>
                            </div>
                        </div>
                    </div>
                    <!-- /Page Header -->
                    <form action="{{ route('stayer.update', $stayer->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{ $stayer->id }}">
                        <input type="hidden" name="old_image"
                            value="{{ $stayer->old_image }}">
                            
                <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group mb-4">
                                <div class="text-center border-0 mt-4 px-2 w-25 mx-auto">
                                    <img src="{{ !empty($stayer->stayer_image) ? url($stayer->stayer_image) : url('upload/no_image.jpg') }}"
                                    ...
                                    <label class="btn btn-outline-success rounded-1 py-1  mx-auto mt-2"
                                        ...
                                        <input type="file" size="60" style="display: none" name="stayer_image"
                            ...
                            <div class="card-body rounded">
                                <div class="form-group">
                                    ...
                                    value="{{ $stayer->name }}" placeholder="បញ្ចូលឈ្មោះ">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select" id="gender" name="gender">
                                        ...
                                        <option value="ប្រុស" {{ $stayer->gender == 'ប្រុស' ? 'selected' : '' }} >ប្រុស</option>
                                        <option value="ស្រី"
                                        {{ $stayer->gender == 'ស្រី' ? 'selected' : '' }}>ស្រី</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>ថ្ងៃចូល</label>
                                    <input type="date" class="form-control" id="date_in" name="date_in"
                                    value="{{ $stayer->date_in }}">
                                </div>
                                {{-- <div class="form-group">
                                    <label>ថ្ងៃចេញ</label>
                                    <input type="date" class="form-control" id="date_out" name="date_out"
                                    value="{{ $stayer->date_out }}">
                                </div> --}}
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="card-body rounded">
                                <div class="form-group">
                                    ...
                                    value="{{ $stayer->old }}" placeholder="បញ្ចូលអាយុ">
                                </div>
                                <div class="form-group">
                                    <label>ទីកន្លែងកំណើត</label>
                                    <textarea class="form-control" id="address" name="address">{{  $stayer->address  }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>តួនាទី</label>
                                    <select class="form-control select" name="role" id="role">
                                        <option selected="" disabled="" value="" disabled>ជ្រើសរើសតួនាទី </option>
                                        <option value="ឩបាសក" {{ $stayer->role == 'ឩបាសក' ? 'selected' : '' }} >ឩបាសក</option>
                                        <option value="ឩបាសិកា" {{ $stayer->role == 'ឩបាសិកា' ? 'selected' : '' }} >ឩបាសិកា</option>
                                        <option value="ដូនជី" {{ $stayer->role == 'ដូនជី' ? 'selected' : '' }} >ដូនជី</option>
                                        <option value="យាយជី" {{ $stayer->role == 'យាយជី' ? 'selected' : '' }} >យាយជី</option>
                                        <option value="តាជី" {{ $stayer->role == 'តាជី' ? 'selected' : '' }} >តាជី</option>
                                    </select>
                                    @error('role')
                                    <div class="form-error text-danger">
                                        {{ $message }}
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="phone" name="phone"
                                        value="{{  $stayer->phone  }}" placeholder="បញ្ចូលលេខទូរស័ព្ទ">
                                </div>
                                <div class="form-group">
                                    <label>ផ្សេងៗ</label>
                                    <textarea class="form-control" id="note" name="note">{{  $stayer->note  }}</textarea>
                                </div>
                            </div>
                        </div>
                ...
                <div class="submit-section">
                    <a href="{{ route('all.stayer') }}"  class="btn btn-danger submit-btn mb-4">ថយក្រោយ</a>
